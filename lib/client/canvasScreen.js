    ...
    var createScreen = function(o) {
        ...
        var res = {
            ...
            onFrame:    function() {
                ...
                for (var i = 0, f = this.shapes.length; i < f; ++i) {
                    s = this.shapes[i];
                    if ('draw' in s) {          // HAS DRAW FN? CALL IT PASSING CTX
                        ctx.save();
                            if (!('static' in s)) {
                    ...
                    x = Math.round(s.pos[0] - this.scroll[0]);
                    y = Math.round(s.pos[1] - this.scroll[1]);
                    w = s.dims ? s.dims[0] : 0;
                    h = s.dims ? s.dims[1] : 0;

                    if (s.color === 'transparent') {    // COLOR IS TRANSPARENT? SHOW STROKE IIF DEBUG
                        if (this.debug) {
                    ...
                    else if ('color' in s) {            // COLOR?
                        if ('repeat' in s) {            // REPEAT? ...
                            ctx.save();
                            ctx.translate(x, y);
                    ...
                    else if ('image' in s) {            // IMAGE? SHOW AS TILED TEXTURE
                        if (s.repeat) {
