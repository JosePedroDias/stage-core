...
(function() {
    ...
    judge.prototype = {
            ...
            if ('code' in o && o.code.length > 0) {
                ...
                session.context = {
                    ...
                    log:      function(session, msg, c) {
                        ...
                        this._stage.send('message', 'LOG: ' + msg, session._socket);
                    }.bind(this, session)
                };
                ...
                try {
                    ...
                    this._stage.send('message', ex, session._socket);
        ...
        /**
         * use this to populate bots perceptions with data from session/state/computed
         */
                        ...
                        for (var i = 0, f = sessionsArray.length, s; i < f; ++i) {
                            ...
                            if (updateBotsFrame && s.isBot) {
                                ...
                                try {
                                    ...
                                    ex = 'CODE ERR: ' + ex.toString();
                                    this._stage.send('message', ex, s._socket);
                                    delete s.isBot;
                                    delete s.script;
                                    console.log(ex);
                                }
        ...
        /**
         * if defined, this function sorts the array of players (use it to force a certain order)
         */
        sortFn: undefined,

        /**
         * returns the object that gets sent to clients via roster message.
         */
        rosterView: function(o) {
            return {
                name:   o.name + (o.isBot ? ' (bot)' : ''),
                id:     o._id
            };
        },

        /**
         * default session data. provide additional defaults and/or random data if it makes sense here...
         */
        ...
        /**
         * Called twice: with false prior to any player entering, with true just before a game starts.
         */
